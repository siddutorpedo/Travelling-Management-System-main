...
export const updateProfilePhoto = async (req, res) => {
  try {
    ...
    return res.status(500).send({
      success: false,
      message: "Internal Server Error during profile photo update",
      error: error.message,
    });
  }
};
...
export const deleteUserAccount = async (req, res, next) => {
  ...
  try {
    ...
    res.clearCookie("X_TTMS_access_token"); // Correct cookie name
    res.status(200).send({
      success: true,
...
export const deleteUserAccountAdmin = async (req, res, next) => {
  try {
    ...
    return res.status(500).send({
      success: false,
      message: "Internal Server Error during admin user deletion",
      error: error.message,
    });
  }
};
